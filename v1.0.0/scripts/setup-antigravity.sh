...
EOF
  echo -e "${RESET}"
  echo -e "  ${BOLD}Antigravity AI Agent — Scaffold Setup v2.0${RESET}"
  echo -e "  ${BOLD}3-Phase System: Requirements → Design → Tasks${RESET}"
...
HOOK
  ...
  # 3-b. Post-task hook (giữ nguyên logic)
  cat > "$AG/hooks/post-task.sh" <<'HOOK'
#!/usr/bin/env bash
# post-task.sh — Chạy sau mỗi task
# Usage: bash hooks/post-task.sh "<task_slug>" "<status: success|failed>"

set -euo pipefail
TASK="${1:-unknown}"; STATUS="${2:-success}"
TS=$(date '+%Y%m%d-%H%M%S')
LOG=".antigravity/logs/agent.log"
...
echo "✅ Post-task hook complete. Log updated."
HOOK
...
# Workflow: Standard Task (v2.0 — 3-Phase)

## Tổng quan
Quy trình chuẩn cho mọi task trong Antigravity: **Requirements → Design → Tasks → Execute → Review**.
...
create_logs() {
  ...
  echo "[${ts}] [INIT] Antigravity scaffold v2.0 (3-Phase) initialized at $ROOT" > "$AG/logs/agent.log"
  log "logs/agent.log"
}
...
YAML
  ...
  cat > "$AG/README.md" <<'MD'
# Antigravity AI Agent (v2.0 — 3-Phase System)

Agent hỗ trợ với quy trình **Requirements → Design → Tasks → Execute → Review**.
...
│   ├── project.context.json  # ← Steering data ở đây
│   ├── session.context.json
│   ├── decisions.log.md
│   └── glossary.md
├── templates/                # Mẫu tài liệu (3-Phase)
...
├── context/                  # Requirements files (tạo động)
└── logs/
    └── agent.log
```
...
# Xem trạng thái 3-phase
bash .antigravity/commands/status.sh

# Bắt đầu task mới (tạo scaffold 3 file)
...
1. 📋 **3-Phase bắt buộc** — Requirements → Design → Tasks trước khi code
2. 👤 **User Stories + AC** — Mọi yêu cầu đều chuẩn PROD
3. 🏗️ **Design trước code** — Architecture, File Map, Technical Decisions
4. ✅ **Task Breakdown** — Mỗi task liên kết User Story, cập nhật [x] live
5. ✋ **Approve từng phase** — Không skip, không giả định
6. 📝 **Ghi log mọi thứ** — Mọi hành động đều có audit trail
7. 🧠 **Steering data** — Tuân thủ architecture, coding style, tech stack
MD
  log "README.md"
}
...
main() {
  ...
  echo -e "${BOLD}${GREEN}  ✅ Antigravity scaffold v2.0 (3-Phase) hoàn tất!${RESET}"
  echo -e "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
  echo ""
  echo -e "  📁 Root:    ${YELLOW}$ROOT${RESET}"
  ...
  echo -e "  3. Bắt đầu task đầu:       ${CYAN}bash $AG/commands/new-task.sh \"\<mô tả\>\"${RESET}"
  echo ""
}
